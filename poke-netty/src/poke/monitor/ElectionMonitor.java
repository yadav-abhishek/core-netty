...
public class ElectionMonitor {
	 ...
	 */
	public ElectionMonitor(String whoami, String host, int port) {
		this.whoami = whoami;
		this.host = host;
		this.port = port;
	...
	public void release() {
		logger.warn("ElectionMonitor releasing resources");
	...
	public boolean startElection() {
		...
		try {
			...
			if (ch.isOpen()) {
				// can i cast socka?
				SocketAddress socka = ch.remoteAddress();
				if (socka != null) {
					InetSocketAddress isa = (InetSocketAddress) socka;
					logger.info("Adding Outgoing channel which adds data in outgoingED of Election Manager");
					ElectionManager.getInstance().addOutgoingChannel(this.whoami, isa.getHostName(),
							isa.getPort(), ch, socka);
				}
			} else
				logger.warn(ch.remoteAddress()+ " not writable");
           /*
	...
	public static class MonitorClosedListener implements ChannelFutureListener {
		private ElectionMonitor monitor;

		public MonitorClosedListener(ElectionMonitor monitor) {
