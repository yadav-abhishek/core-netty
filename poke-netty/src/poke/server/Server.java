...
import poke.server.management.managers.ElectionConnector;
...
public class Server {
	...
	public void release() {
		...
		if (ElectionManager.getInstance() != null)
			ElectionManager.getInstance().release();
	...
	private void startManagers() {
		...
		electionMgr = ElectionManager.getInstance(myId);
		
		
		for (NodeDesc nn : conf.getNearest().getNearestNodes().values()) {
			ElectionData node = new ElectionData(nn.getNodeId(), nn.getHost(), nn.getPort(), nn.getMgmtPort());
			ElectionConnector.getInstance().addConnectToThisNode(node);
		}

		// create manager for accepting jobs
		jobMgr = JobManager.getInstance(myId);

		// establish nearest nodes and start receiving heart beats
		...
		// omkar did this bad bad thing
		
		electionMgr.start();
		
		ElectionConnector econn = ElectionConnector.getInstance();
		econn.start();
